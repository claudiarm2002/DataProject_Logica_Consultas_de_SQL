...
-- CORRECCIÓN
select  f."length", f."film_id", f."title"
from "film" f
...
-- CORRECCIÓN
-- I select the third last element
with third_last as (
    select r."rental_id"
    from rental r
...
-- I create a table containing all the payments for the third last rental, and then I sum these payments to calculate the total cost of that rental.
select SUM(p.amount) as total_cost_third_last
from payment p
join third_last t on p.rental_id = t.rental_id;
...
-- CORRECTION (in the temporary table)
select distinct a."first_name", a."last_name"
...
-- CORRECCIÓN
-- Using CROSS JOIN to generate all possible combinations (cartesian product) between staff and stores,
-- since there is no inherent relationship between a staff member and a store in this query.
select 
